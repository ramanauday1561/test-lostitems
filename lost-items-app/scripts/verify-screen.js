...
const SPECS = {
  registry: [
    ...
    ['Item card 60px gradient well', /width: 60,\s*\n\s*height: 60,/],
  ...
  detail: [
    ['196px hero image well', /height: 196,/],
    ['Well gradient #F4F4F2 -> #E9E9E5', /'#F4F4F2', '#E9E9E5'/],
  ...
  shell: [
    ['Mono kicker, uppercase, ink-faint', /font-mono text-\[10px\] uppercase[\s\S]{0,40}text-ink-faint/],
    ['Header title 800 26px/1.12', /text-\[26px\] leading-\[29px\] tracking-\[-0\.78px\]/],
    ['Inbox button 44px w/ unread badge', /h-11 w-11[\s\S]{0,400}unread > 0/],
    ['Avatar shows derived initials', /initials\(session\?\.username/],
    ['Nav pill: white, radius 28, padding 6', /rounded-panel bg-surface p-1\.5/],
    ['Nav tab 52px, active canvas bg + blue', /min-h-\[52px\][\s\S]{0,200}bg-canvas/],
    ['Report FAB 52x52, radius 20, blue', /h-\[52px\] w-\[52px\][\s\S]{0,60}rounded-btn bg-primary/],
    ['FAB hidden for admins', /!admin && \(/],
    ['Member tabs Home/Lost/Found/Forum', /MEMBER_LEFT[\s\S]{0,400}MEMBER_RIGHT/],
    ['Admin tabs Home/Registry/Review/Members/Forum', /ADMIN_LEFT[\s\S]{0,500}ADMIN_RIGHT/],
  ],
  dashboard: [
    ['Search-the-registry entry, tune chip', /Search the registry[\s\S]{0,200}name="tune"/],
    ['3 stat tiles from dashStats', /dashStats[\s\S]{0,1600}s\.value/],
    ['Stat number 800 24px', /text-\[24px\] tracking-\[-0\.96px\]/],
    ['"Recently handed in" + See all', /Recently handed in[\s\S]{0,400}See all/],
    ['Horizontal rail, 200px cards', /horizontal[\s\S]{0,900}w-\[200px\]/],
    ['Rail card 124px well, 44px glyph', /height: 124[\s\S]{0,120}size=\{44\}/],
    ['Floating status chip over the well', /absolute right-2 top-2[\s\S]{0,120}\$\{hue\}1A/],
    ['"Browse all" tail card', /Browse all/],
    ['3 shortcut rows, 76px, blue chip', /SHORTCUTS\.map[\s\S]{0,300}min-h-\[76px\]/],
    ['Messages shortcut shows unread count', /unread from people returning your items/],
    ['Community comments w/ mono initials', /COMMUNITY_COMMENTS\.map/],
  ],
};
