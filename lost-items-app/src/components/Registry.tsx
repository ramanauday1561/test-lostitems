...
import {
  REGISTRY_FILTERS, filterRegistry, type ItemKind, type ItemListRow, type RegistryFilter,
} from '../data/mockData';
import { SHADOW, SHADOW_PILL_OFF, SHADOW_PILL_ON, SHADOW_SEARCH, SHADOW_SEG_ON } from '../design';
import { Icon } from './Icon';
import { ItemCard } from './ItemCard';

export const Registry = ({ kind }: { kind: ItemKind }) => {
  ...
  return (
    <ScrollView keyboardShouldPersistTaps="handled">
        ...
        <View className="flex-row gap-1 rounded-[24px] bg-track p-1">
          {(['lost', 'found'] as const).map((k) => {
            ...
            return (
              <Pressable
                key={k}
                onPress={() => router.navigate(`/${k}` as never)}
      ...
      <View className="gap-2 px-5 pb-8 pt-3">
        ...
        {myPostsEmpty && (
          <View className="items-center rounded-panel bg-surface px-6 py-[44px]" style={{ boxShadow: SHADOW.raised }}>
            ...
            <Text className="mt-4 font-sans-bold text-[15px] text-ink">You haven&apos;t posted anything</Text>
            <Text className="mb-[18px] mt-1.5 max-w-[240px] text-center font-sans text-[13px] leading-[20px] text-ink-soft">
              Your reports show up here, and you can update their status once the item is handed over.
            </Text>
            <Pressable
